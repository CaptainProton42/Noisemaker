...
onready var shader_material = get_node("Plane").get_surface_material(0)
...
func _ready():
	...
	get_node("Plane").get_surface_material(0).set_shader_param("volume", noise_tex)

func _process(delta):
	var bMin = get_node("Cloud").translation - get_node("Cloud").scale / 2.0
	var bMax = get_node("Cloud").translation + get_node("Cloud").scale / 2.0
